...
export default function GitHubProjectCard({ project }: GitHubProjectCardProps) {
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-gray-100 overflow-hidden group">
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.topics.slice(0, 4).map((topic, index) => (
            <span 
              key={index}
              className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full hover:bg-[#6366F1] hover:text-white transition-colors duration-300 cursor-pointer"
            >
              {topic}
            </span>
          ))}
